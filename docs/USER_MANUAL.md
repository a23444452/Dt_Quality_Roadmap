...
| 條件 | 說明 | 如何取得 |
|------|------|---------|
| Corning Azure AD 帳號 | 公司 Microsoft 帳號 (yourname@corning.com) | 入職時自動配發 |
| AD Group 成員資格 | 必須加入 `Quality-Roadmap-Access` AD Group | 請聯繫系統管理員或您的主管申請加入 |

> **重要**: 若您不是 `Quality-Roadmap-Access` AD Group 的成員，系統將拒絕存取 (顯示 403 Access Denied)。

### 1. 登入系統

本系統採用 **Microsoft SSO (Single Sign-On)** 登入，無需另外建立帳號密碼。

1. 進入系統登入頁面
2. 點選 **Sign in with Microsoft** 按鈕
3. 系統會導向 Microsoft 登入頁面，使用您的 Corning 帳號 (yourname@corning.com) 登入
4. 首次登入時，系統會提示您完成註冊 (見下方步驟)
5. 已註冊且已核准的用戶，登入成功後會自動導向 Process Map 首頁

### 2. 首次註冊

首次使用 SSO 登入時，系統會偵測到您尚未註冊，並自動彈出註冊對話框：

1. 點選 **Sign in with Microsoft** 完成 SSO 驗證
2. 系統會自動帶入您的帳號、Email、顯示名稱
3. 在註冊對話框中選擇：
   - **Plant**: 選擇您負責的工廠
   - **Process**: 選擇您負責的製程
4. 點選 **Register** 提交申請
5. 等待管理員審核通過後，下次 SSO 登入即可正常使用

> **注意**: 註冊後帳號狀態為「待審核 (Pending)」，需管理員核准後才能登入。核准前再次登入會顯示「Your account is awaiting administrator approval.」

### 3. 登出與重新登入

1. 點選右上角使用者名稱，選擇 **Logout**
2. 重新登入時點選 **Sign in with Microsoft** 即可
...
### G$ Management (Goal Sharing 管理)

管理 Goal Sharing (G$) 項目的新增、編輯和追蹤。

**使用方式**:

1. **篩選**: 使用頁面上方的篩選列，依 Plant 或 Process 篩選 G$ 項目
2. **瀏覽**: 分頁表格列出所有 G$ 項目，包含以下資訊：
   - Solution 名稱
   - 所屬 Plant / Process / Station
   - 目標日期與狀態
3. **編輯** (Editor/Admin 限定): 點選項目開啟編輯對話框，可修改狀態和相關資訊

**權限限制**: Editor 只能編輯被分配 Plant 與 Process 交集的 G$ 項目。

---

### G$ Tracking (Goal Sharing 追蹤儀表板)

提供 G$ 項目的完成進度追蹤與視覺化分析，呈現年度 Goal Sharing 目標達成狀況。

**功能區塊**:

#### 1. Status Table (狀態總覽表)
列出所有 D^t 項目的執行狀態：

| 欄位 | 說明 |
|------|------|
| Solution | D^t Solution 名稱 |
| Status | 完成狀態 (Complete / In Progress) |
| Plant | 所屬工廠 |
| Complete Date | 完成日期 |
| Line/Tank | 所屬產線 |
| Category | 缺陷類別 |

#### 2. Monthly Trend Chart (月趨勢圖)
- **長條圖**: 每月實際累計完成數量 (Actual)
- **折線**: Budget 目標線與 Stretch 目標線
- 可快速判斷當月進度是否符合目標

#### 3. Solution Bar Chart (類別分析圖)
- 水平堆疊長條圖，依 Defect Category 分類
- 每個類別顯示 Complete (已完成) 與 Not Complete (未完成) 的數量
- 快速掌握各類別的完成比例

#### 4. Plant Target Chart (工廠目標圖)
- 依工廠分組的堆疊長條圖
- 疊加 Budget 與 Stretch 目標水平線
- 對比各工廠的完成數量與年度目標

**使用方式**:
1. 進入 G$ Tracking 頁面
2. 查看 Status Table 了解個別項目狀態
3. 透過 Monthly Trend Chart 追蹤時間進度
4. 利用 Solution Bar Chart 分析各類別完成情況
5. 透過 Plant Target Chart 比較各工廠表現
...
### Q2: 登入時出現 "Access denied" 怎麼辦？
A: 這代表您不是 `Quality-Roadmap-Access` AD Group 的成員。請聯繫系統管理員或您的主管，申請將您加入該 AD Group。

### Q3: 登入時卡在 "Signing in..." 怎麼辦？
A:
1. 清除瀏覽器快取和 Session Storage
2. 關閉所有瀏覽器視窗後重新開啟
3. 重新進入系統並點選 **Sign in with Microsoft**
4. 若問題持續，請聯繫系統管理員

### Q4: 為什麼我無法編輯 Solution Map 的某些儲存格？
A: 可能原因：
- 您的角色是 Viewer，無編輯權限
- 該儲存格不在您被分配的 Plant 或 Process 範圍內
- 請聯繫管理員調整您的權限設定

### Q5: 如何查看我被分配的 Plant 和 Process？
A: 
1. 點選右上角的使用者名稱
2. 進入 **Profile** 頁面
3. 查看帳號資訊區塊
